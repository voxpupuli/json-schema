!https://travis-ci.org/ruby-json-schema/json-schema.svg?branch=master!:https://travis-ci.org/ruby-json-schema/json-schema !https://codeclimate.com/github/ruby-json-schema/json-schema/badges/gpa.svg!:https://codeclimate.com/github/ruby-json-schema/json-schema
...
h2. Version 3.0.0 Upgrade Notes

This upgrade drops several features from 2.x in order to avoid ambiguities w.r.t. the spec and focus on what json-schema is: a validator for JSON schemas. A detailed list of changes can be found in the [changelog](Changelog.md). Noteworthy changes:

* Data has to be JSON-like (i.e. json-schema won't parse data, load data from URIs or convert symbols in data)
...
All methods take two arguments. The first argument to these methods is always the schema, the second is always the data to validate. An optional third options argument is also accepted; available options are used in the examples below.
The schema can be given as a URI, from which the schema will be fetched then or a Ruby object representing the schema.
Data must always be given as Ruby object representing JSON data (symbols are not considered as valid strings).
